...
describe('dependency-manager', function () {
	it('should load without a dependency', async function () {
		...
		dm.register<string, string>('foo', libStub);

		await dm.load();

		expect(dm.get<string>('foo')).to.equal('instance-value');
	...
	it('should load with a dependency', async function () {
		...
		dm.register<string, string>('foo', sinon.stub().resolves('foo-value'));
		dm.register<string, string>('bar', barStub, ['foo']);

		await dm.load();

		expect(dm.get<string>('foo')).to.equal('foo-value');
		expect(dm.get<string>('bar')).to.equal('bar-value');
	...
	it('should load with a dependency using the argument injection', async function () {
		...
		dm.register<string, string>('foo', sinon.stub().resolves('foo-value'));
		dm.register<string, string>('baz', sinon.stub().resolves('baz-value'));
		dm.register<string, string>('bar', barStub, ['foo', 'baz']);
	...
	it('should load with a dependency using snake case', async function () {
		...
		dm.register<string, string>('foo-dep', sinon.stub().resolves('foo-value'));
		dm.set<string>('bazDep', 'baz-value');
		dm.set<string>('lib:fooBar', 'foobar-value');
		dm.alias('bazDep', 'baz-alias');
		dm.register<string, string>('bar', barStub, ['foo-dep', 'bazDep', 'lib:fooBar', 'baz-alias']);
	...
	it('should load with a dependency using camelCase', async function () {
		...
		dm.register<string, string>('foo-dep', sinon.stub().resolves('foo-value'));
		dm.set<string>('baz_dep', 'baz-value');
		dm.alias('baz_dep', 'baz-alias');
		dm.register<string, string>('bar', barStub, ['foo-dep', 'baz_dep', 'baz-alias']);
	...
	it('should load with a dependency using namespaces', async function () {
		...
		dm.register<string, string>('lib:foo-dep', sinon.stub().resolves('foo-value'));
		dm.set('lib:baz_dep', 'baz-value');
		dm.alias('lib:baz_dep', 'lib:baz-alias');
		dm.register<string, string>('lib:bar', barStub, ['lib:foo-dep', 'lib:baz_dep', 'lib:baz-alias']);
	...
	it('should load with same dependency in multiple dependencies', async function () {
		...
		dm.register<string, string>('foo', sinon.stub().resolves('foo-value'));
		dm.register<string, string>('bar', barStub, ['foo']);
		dm.register<string, string>('baz', bazStub, ['foo']);
	...
	it('should set a static instance', async function () {
		...
		dm.set<string>('static-name', 'static-value');

		await dm.load();

		expect(dm.get<string>('static-name')).to.equal('static-value');
	});
	...
	it('should load with a static instance dependency', async function () {
		...
		dm.set<string>('staticName', 'static-value');
		dm.register<string, string>('bar', barStub, ['staticName']);
	...
	it('should load an alias', async function () {
		...
		expect(dm.get<string>('bar')).to.equal('instance-value');
	});
	...
	it('should load with an alias as a dependency', async function () {
		...
		dm.register<string, string>('foo', sinon.stub().resolves('foo-value'));
		dm.alias('foo', 'bar');
		dm.register<string, string>('baz', bazStub, ['bar']);
	...
	it('should load a dependency with a namespace', async function () {
		...
		dm.register<string, string>('lib:foo', sinon.stub().resolves('foo-value'));
		dm.register<string, string>('bar', barStub, ['lib:foo']);

		await dm.load();

		expect(dm.get<string>('lib:foo')).to.equal('foo-value');
		expect(dm.get<string>('bar')).to.equal('bar-value');
		expect(barStub).to.be.calledWithExactly({fooLib: 'foo-value'});
	});

	it('should call onLoaded callback when dependency is loaded', async function () {
		const dm = new DependencyManager();
		const barStub = sinon.stub().resolves('bar-value');
		const libFooLoadedStub = sinon.stub();
		const barLoadedStub = sinon.stub();

		dm.register<string, string>('lib:foo', sinon.stub().resolves('foo-value'), libFooLoadedStub);
		dm.register<string, string>('bar', barStub, ['lib:foo'], barLoadedStub);

		await dm.load();

		expect(libFooLoadedStub).to.be.calledWithExactly('foo-value');
		expect(barLoadedStub).to.be.calledWithExactly('bar-value');
	});

	it('should error if trying to get a dependency before load', async function () {
		const dm = new DependencyManager();
		expect(() => dm.get<string>('foo')).to.throw('Attempt to get dependency before load');
	});

	it('should error if trying to get a dependency that does not exists', async function () {
		const dm = new DependencyManager();
		await dm.load();
		expect(() => dm.get<string>('does-not-exist')).to.throw('does-not-exist is not registered as a dependency');
	});

	it('should error if trying to load a dependency that does not exists', async function () {
		const dm = new DependencyManager();
		dm.register<string, string>('bar', sinon.stub(), ['does-not-exist']);
	...
	it('should error if trying to load a nested dependency that does not exists', async function () {
		const dm = new DependencyManager();
		dm.register<string, string>('foo1', sinon.stub(), ['foo2']);
		dm.register<string, string>('foo2', sinon.stub(), ['foo3']);
		dm.register<string, string>('foo3', sinon.stub(), ['foo4']);
		dm.register<string, string>('foo4', sinon.stub(), ['foo5']);
		dm.register<string, string>('foo5', sinon.stub(), ['does-not-exist']);
	...
	it('should error on cyclic dependency', async function () {
		const dm = new DependencyManager();
		dm.register<string, string>('bar', sinon.stub(), ['foo']);
		dm.register<string, string>('foo', sinon.stub(), ['bar']);
	...
	it('should error on adding a alias that conflicts with a dependency', async function () {
		const dm = new DependencyManager();
		dm.register<string, string>('bar', sinon.stub());
	...
	it('should error on adding a constant value that conflicts with a dependency', async function () {
		const dm = new DependencyManager();
		dm.register<string, string>('bar', sinon.stub());
		expect(() => dm.set<string>('bar', 'value')).to.throw(
	...
	it('should error on adding a dependency that was already registered', async function () {
		const dm = new DependencyManager();
		dm.register<string, string>('foo', sinon.stub());
	...
	it('should error on setting a constant that was already set', async function () {
		const dm = new DependencyManager();
		dm.set<string>('bar', 'bar-value');
		expect(() => dm.set<string>('bar', 'bar-new-value')).to.throw(
			'Unable to set constant value, bar, because it conflicts with dependency, bar'
		);
	});
	...
	it('should error on adding a dependency with a similar name that was already registered', async function () {
		const dm = new DependencyManager();
		dm.register<string, string>('foo-bar', sinon.stub());
	...
	it('should error on setting a constant with a similar name that was already set', async function () {
		const dm = new DependencyManager();
		dm.set<string>('foo-bar', 'bar-value');
		expect(() => dm.set<string>('foo_bar', 'bar-new-value')).to.throw(
	...
	it('should error if trying to load itself', async function () {
		const dm = new DependencyManager();
		dm.register<string, string>('bar', sinon.stub(), ['bar']);
	...
	it('should error if trying to load with an alias in a dependency cycle', async function () {
		const dm = new DependencyManager();
		dm.register<string, string>('a', sinon.stub(), ['bb']);
		dm.register<string, string>('b', sinon.stub(), ['cc']);
		dm.register<string, string>('c', sinon.stub(), ['aa']);
		dm.alias('a', 'aa');
		dm.alias('b', 'bb');
		dm.alias('c', 'cc');
	...
	it('should error when adding a dependency with an invalid name', async function () {
		const dm = new DependencyManager();
		expect(() => dm.register<string, string>('foo{bar', sinon.stub()))
			.to.throw('The name, foo{bar, is not a valid identifier');
	});

	it('should error when adding a namespace with an invalid name', async function () {
		const dm = new DependencyManager();
		expect(() => dm.register<string, string>('{foo:bar', sinon.stub()))
			.to.throw('The namespace, {foo, is not a valid identifier');
	});
});
